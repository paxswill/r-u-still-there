...
#[cfg(feature = "mozjpeg")]
fn encode_jpeg_mozjpeg(image: &BytesImage) -> anyhow::Result<Bytes> {
    ...
    let jpeg_bytes = std::panic::catch_unwind(|| {
        ...
        jpeg_encoder.data_to_vec().unwrap()
    })
    .map_err(|_| anyhow!("Error encoding MJPEG with mozjpeg"))?;
    Ok(Bytes::from(jpeg_bytes))
}

fn encode_jpeg_image(image: &BytesImage) -> anyhow::Result<Bytes> {
    ...
    Ok(jpeg_buf.into_inner().freeze())
}

#[cfg(not(feature = "mozjpeg"))]
pub(crate) fn encode_jpeg(image: &BytesImage) -> anyhow::Result<Bytes> {
